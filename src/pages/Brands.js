    ...
    <div className="min-h-screen bg-slate-100 p-6">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-sky-700 rounded-3xl p-8 mb-8 text-white shadow-xl">
        <div className="flex items-center justify-between flex-wrap gap-5">
          <div>
            <h1 className="text-4xl font-bold mb-2">Principals Management</h1>
            <p className="text-slate-200">Manage all laboratory Principals</p>
          </div>
          <div className="bg-white/10 backdrop-blur-md rounded-2xl px-6 py-4">
            <h2 className="text-3xl font-bold">{brands.length}</h2>
            <p className="text-sm text-slate-200">Total Principals</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-lg p-6 mb-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-2xl font-bold">{editId ? "Update Principal" : "Create Principal"}</h2>
          </div>
          {editId && (
            <button
              type="button"
        ...
        <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-5">
          {/* Name */}
          <div>
            <label className="font-semibold text-sm text-slate-600 block mb-2">Principal Name</label>
            <input
              type="text"
              name="name"
              value={formData.name}
          ...
          {/* Logo */}
          <div className="md:col-span-2">
            <label className="font-semibold text-sm text-slate-600 block mb-2">Principal Logo</label>
          ...
          <div className="md:col-span-2 flex gap-3">
            ...
            >
              <Plus size={18} />
              {loading ? "Processing..." : editId ? "Update Principal" : "Create Principal"}
            </button>
            {editId && (
              <button
                type="button"
      ...
      {filteredBrands.length === 0 && (
        <div className="text-center py-16">
          ...
          <h2 className="text-2xl font-bold text-slate-500">No Principals Found</h2>
        </div>
      )}
    </div>
  );
