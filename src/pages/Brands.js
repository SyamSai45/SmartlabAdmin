...
const api = axios.create({ baseURL: "https://smartlabtechbackend.onrender.com/api" });
...
const BrandsPage = () => {
  ...
  const handleEdit = (item) => {
    ...
    setPreview(item.logo ? `https://smartlabtechbackend.onrender.com/${item.logo.replace(/\\/g, "/")}` : "");
    setError("");
  ...
  const logoUrl = (logo) =>
    logo ? `https://smartlabtechbackend.onrender.com/${logo.replace(/\\/g, "/")}` : null;
